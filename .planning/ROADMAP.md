...
- [x] 01-19-PLAN.md — Close G-01-25: correct the shell chrome document's `<title>` and loading wordmark to the spaced display form, correct the same-class stale identifier-form expectation latent in `_branding_variant_divergence_impl`, and derive the shell's chrome markup into the preflight's display-surface leak scan — whose existing pattern already matched both literals and simply never read the file — with a planted fault reproducing the pre-fix title
...
- [x] 02-03-PLAN.md — Mask the defaults layer before the merge, merge recursively with array-replace, and echo every applied default in sorted order
...
- [x] 02-05-PLAN.md — The byte-identity gate: emit to a temp dir, set-compare the target list, seven planted faults, two registry rows
...
- [x] 03-01-PLAN.md — Tracer: spike --with-branding outside topsrcdir plus identity carrier, then locale emitters with ftl/properties agreement, literal layout rows, agreement registry check, and the build wiring
...
- [x] 07-02-PLAN.md — Prove the milestone acceptance core with a synthetic downstream: a Sourcerer-equivalent external config yields a fully branded product from an untouched platform tree
- [x] 07-03-PLAN.md — Close VER-03's adversarial half: four hostile fixtures that prove the platform is not accidentally built around one friendly config
- [x] 07-04-PLAN.md — Formalize the both-layers-pass proof, carry-test the rebranding guide against a fixture, and stage the tier-3 drills
...
| 7. Sourcerer as Downstream | 4/4 | In Progress|  |
