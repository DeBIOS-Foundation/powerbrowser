...
Self-hosted MAR updates under fork signing, plus the Nix-built Windows
...
Fork key plus HTTPS plus hash-pinned update descriptors:
...
- MARs are built with the tree's own `mar` binary from an updater-enabled
  ...
  drops the Mozilla-key hard requirement — Mozilla's MAR signing key is
  unobtainable by design, so this flag is the documented fork path, not a
  weakened check of a key we hold.
- Update descriptors are generated by `buildUpdateXml` in
  `scripts/verify-mar-update-hop.mjs` straight from the MAR bytes
  (sha512 plus size) and served over HTTPS in production. The local proof
  serves over plain HTTP on loopback only.
