...
// asserts `window.theia.container.isBound(Symbol.for('SourcererPrivilegedJs'))`
...
// SourcererPrivilegedJs class remains in bundle.js (tree-shaking cannot
...
//
// Current expected state (until Plan 05 lands `@sourcerer/customize`):
...
const CHECK_BINDING = "window.theia.container.isBound(Symbol.for('SourcererPrivilegedJs'))";
...
main()
    .then(isBound => {
        if (expectBound) {
            ...
            console.error(`verify-dev-flag-off: FAIL -- expected SourcererPrivilegedJs to be bound, isBound() returned ${JSON.stringify(isBound)}`);
            process.exit(1);
        }
        if (isBound === false) {
        ...
        console.error('verify-dev-flag-off: FAIL -- SourcererPrivilegedJs is bound with the dev flag off');
        process.exit(1);
    })
    .catch(err => {
